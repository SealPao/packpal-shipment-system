...
class ReturnReceivingWindow(QMainWindow):
    def __init__(self, parent_mode_select=None, selected_camera_name: str | None = None, draft_service: DraftService | None = None) -> None:
        ...
        self.setWindowTitle(f"{APP_TITLE} - 退貨收貨")
        ...
        center_prompt = QLabel("請掃描退貨單號開始收貨")
        ...
        sub_prompt = QLabel("正式版會在這裡顯示相機畫面、退貨原因與外觀檢查流程。")
        ...
        self.scan_input.setPlaceholderText("請掃描或輸入退貨單號")
        ...
        self.setStyleSheet(app_stylesheet("#7c3aed", "#6d28d9") + "QFrame#cameraStage { background: qlineargradient(x1:0,y1:0,x2:1,y2:1, stop:0 #2e1065, stop:1 #6d28d9); }")
    ...
    def load_latest_draft(self) -> None:
        draft = self.draft_service.latest_draft("returns")
